...
(function (RT) {
  ...
  function renderPractice(s, toks, blanks, mode) {
    ...
    while (i < toks.length) {
      ...
      if (t.t === 'br') { html += '\n'; i++; continue; }
      // 已被手选覆盖的空格也是蓝块的一部分，必须先判断 isBlank；
      // 只有普通空格才直接输出，避免把一段连续选区拆成两个词块。
      if (t.t === 'sp' && !isBlank) { html += ' '; i++; continue; }
      if (isBlank) {
        // 合并相邻空白成"一整块"
        var block = [i];
        var j = i + 1;
        // 手选范围是独立的精确对象：不能被相邻的自动蓝块吞并，
        // 否则用户刚调整的左右边界会在视觉上扩展成别的词。
        var isManual = !!t.manual;
        while (j < toks.length && blanks.indexOf(j) >= 0 && !!toks[j].manual === isManual) { block.push(j); j++; }
        bi++;
        ...
        if (mode === 'design') {
          ...
          html += '<span class="tk on' + (isManual ? ' manual' : '') + (forced ? ' forced' : '') + '" data-i="' + i + '" data-block="' + block.join(',') + '" title="' + (forced ? '错题强制重现 · 浏览满5次自动解除' : '已挖空 · 点一下取消') + '">' + inners + '</span>';
