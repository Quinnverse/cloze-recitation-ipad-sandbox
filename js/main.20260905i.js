...
(function (RT) {
  ...
  function bind() {
    ...
    function paperOffsetAtPoint(x, y) {
      ...
      // 不采用 caretRangeFromPoint 的返回值：WebKit 在字符右边界、禁用原生选区
      // 以及手柄覆盖文字时经常返回左侧 caret，视觉边界与数据会稳定相差一个字。
      // 缓存的逐字符矩形既用于画高亮，也用于命中，二者共享同一坐标真值。
    ...
    var handleGrabDX = 0, handleGrabDY = 0; // 手指抓取点到真实文字边界的坐标补偿
    var pendingHandlePoint = null, handleMoveFrame = 0;
    ...
    // 真机 touchmove 的采样频率可能高于屏幕刷新率。只在下一帧处理最后一个坐标，
    // 避免同一帧反复执行 Range.getClientRects + 覆盖层重绘造成卡顿。
    function updateHandleAtPoint(point) {
      if (!point || !activeHandle || !curRange) return;
      var handleOff = paperOffsetAtPoint(point.x, point.y);
      if (handleOff == null) return;
      ...
      if (next === lastHandleOffset) return;
    ...
    function flushHandleMove() {
      handleMoveFrame = 0;
      var point = pendingHandlePoint; pendingHandlePoint = null;
      updateHandleAtPoint(point);
    }
    ...
    document.addEventListener('touchstart', function (e) {
      ...
      if (handle && curRange) {
        ...
        if (handleMoveFrame) { cancelAnimationFrame(handleMoveFrame); handleMoveFrame = 0; }
        // 手柄中心故意画在文字外侧，且点击热区为 40px。记录实际抓取点与
        // 高亮文字边界的差值；后续始终用“真实边界坐标”命中字符，杜绝偏字。
        var highlights = selLayer && selLayer.querySelectorAll('.sel-hl');
        var boundaryRect = highlights && highlights.length ?
          (activeHandle === 'start' ? highlights[0].getBoundingClientRect() : highlights[highlights.length - 1].getBoundingClientRect()) : null;
        if (boundaryRect) {
          var boundaryX = activeHandle === 'start' ? boundaryRect.left : boundaryRect.right;
          var boundaryY = boundaryRect.top + boundaryRect.height / 2;
          handleGrabDX = boundaryX - t.clientX;
          handleGrabDY = boundaryY - t.clientY;
        } else { handleGrabDX = 0; handleGrabDY = 0; }
    ...
    document.addEventListener('touchmove', function (e) {
      ...
      if (activeHandle && curRange) {
        ...
        pendingHandlePoint = { x: ht.clientX + handleGrabDX, y: ht.clientY + handleGrabDY };
        if (!handleMoveFrame) handleMoveFrame = requestAnimationFrame(flushHandleMove);
        suppressPaperClickUntil = Date.now() + 800;
        e.preventDefault();
        return;
      }
    ...
    document.addEventListener('touchend', function (e) {
      ...
      if (activeHandle) {
        ...
        if (endTouch) pendingHandlePoint = { x: endTouch.clientX + handleGrabDX, y: endTouch.clientY + handleGrabDY };
        if (handleMoveFrame) { cancelAnimationFrame(handleMoveFrame); handleMoveFrame = 0; }
        if (pendingHandlePoint) { var finalPoint = pendingHandlePoint; pendingHandlePoint = null; updateHandleAtPoint(finalPoint); }
        activeHandle = null; lastHandleOffset = null; tpAnchor = null; tpAnchorOff = null; tpHitToken = null;
        handleGrabDX = 0; handleGrabDY = 0;
        suppressPaperClickUntil = Date.now() + 800;
        return;
      }
    ...
    document.addEventListener('touchcancel', function () {
      clearTimeout(lpTimer);
      if (handleMoveFrame) { cancelAnimationFrame(handleMoveFrame); handleMoveFrame = 0; }
      pendingHandlePoint = null; handleGrabDX = 0; handleGrabDY = 0;
