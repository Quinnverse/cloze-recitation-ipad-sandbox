...
(function (RT) {
  ...
  function enterPractice(s) {
    ...
    // 触屏设备：先确定划词模式初值（在 renderPractice 之前），让 renderPractice 能据此回写 touch-sel 类。
    // 真正加 touch-sel 类由 renderPractice 依据 touchSelectMode 回写，避免重渲染丢失。
    if (isTouchDevice()) {
      // 触屏默认【不开】划词模式：滚动优先。选词解耦靠 touchmove 的「方向判定」——纵向滑=滚动，
      // 横向/对角拖、或长按 350ms、或手动开开关=选词。这样默认关模式下 iPad 横拖也能选词
      // （绕开 iOS 长按被系统接管），而 vivo/iPad 上下滑都能正常滚动（不再被 preventDefault 锁死）。
      touchSelectMode = false;
      var tsBtn = $('#btnTouchSel'); if (tsBtn) { tsBtn.hidden = false; } // 仅展示开关，默认 off
    }
    go('practice');
    renderPractice();
  ...
  function bind() {
    ...
    function paintHighlight(aNode, aOff, bNode, bOff) {
      ...
      // 计算字符偏移（用于真正挖空）
      var so = resolveOffset(aNode, aOff), eo = resolveOffset(bNode, bOff);
    ...
    document.addEventListener('touchstart', function (e) {
      ...
      tpAnchor = caretPoint(t.clientX, t.clientY) || caretPointByRects(t.clientX, t.clientY);
      // 清除上一次残留高亮
      clearHighlight();
      ...
      lpTimer = setTimeout(function () {
        ...
        // 长按不必再移动：直接把当前 token 变成业务选区并显示“挖空”浮层。
        // 这完全使用覆盖层，不创建原生 Selection，因此不会唤起复制/翻译。
        var anchorEl = tpAnchor && (tpAnchor.node.nodeType === 1 ? tpAnchor.node : tpAnchor.node.parentElement);
        var tokenEl = tpHitToken || (anchorEl && closestEl(anchorEl, '.tk'));
        if (tokenEl && p && p.contains(tokenEl)) {
          tokenEl.classList.add('longpress-hit');
          paintHighlight(tokenEl, 0, tokenEl, tokenEl.childNodes.length);
          setTimeout(function () { tokenEl.classList.remove('longpress-hit'); }, 500);
        } else if (tpAnchor && tpAnchor.node.nodeType === 3 && tpAnchor.node.nodeValue.length) {
          var charEnd = Math.min(tpAnchor.offset + 1, tpAnchor.node.nodeValue.length);
          paintHighlight(tpAnchor.node, tpAnchor.offset, tpAnchor.node, charEnd);
        } else {
          // iOS 若命中普通文本且不给 caret 锚点，至少给出明确反馈，避免用户误以为长按失效。
          RT.util.toast('已识别长按，请从带下划线或蓝色词块开始拖动');
        }
      }, 350);
    }, { passive: true });
    ...
    document.addEventListener('touchmove', function (e) {
      if (state.view !== 'practice' || state.mode !== 'design') return;
      var t = e.touches && e.touches[0]; if (!t || !tpAnchor) return;
      var dx = t.clientX - tpStartX, dy = t.clientY - tpStartY;
      // 阈值内：只等待是否触发长按，暂不定意图（避免轻微抖动误判）
      if (Math.abs(dx) < 8 && Math.abs(dy) < 8) return;
      // 已锁定为「选词态」：继续画高亮（纵向拖也照画，支持跨行/跨段选词）
      if (tpDragging) {
        e.preventDefault();
        var cur2 = caretPoint(t.clientX, t.clientY); if (!cur2) return;
        paintHighlight(tpAnchor.node, tpAnchor.offset, cur2.node, cur2.offset);
        return;
      }
      // 首次越阈值：一次性判定意图并锁定，之后不再因方向抖动而切换
      //   纵向滑动为主(|dy|>|dx|) → 视为滚动，放行浏览器原生滚动，绝不 preventDefault；
      //   横向/对角拖，或已长按 350ms，或手动开了「划词模式」 → 视为选词。
      // 效果：vivo/iPad 在正文上随意上下滑都可滚动（不再锁死）；iPad 横向拖字即可选词，
      //       无需依赖长按（绕开 iOS Safari 长按被系统放大镜/查询接管、计时器来不及触发的问题）。
      var vertical = Math.abs(dy) > Math.abs(dx);
      if (tpLongPress || isTouchSelect() || !vertical) {
        tpDragging = true; clearTimeout(lpTimer);
        suppressPaperClickUntil = Date.now() + 800;
        e.preventDefault();
        var cur = caretPoint(t.clientX, t.clientY); if (!cur) return;
        paintHighlight(tpAnchor.node, tpAnchor.offset, cur.node, cur.offset);
      } else {
        // 纵向滚动：放行，并清长按计时，避免误进入选词态
        clearTimeout(lpTimer);
      }
    }, { passive: false });
    ...
    document.addEventListener('touchend', function (e) {
      ...
      if (tpDragging) {
        // 选区保留（高亮+selPop 仍在），等用户点按钮；仅复位拖选/长按标志
        tpDragging = false; tpLongPress = false; tpAnchor = null; tpHitToken = null; return;
      }
      // 未拖选：长按但未移动 → 清除；或点空白收起浮层
      tpLongPress = false; tpAnchor = null; tpHitToken = null;
