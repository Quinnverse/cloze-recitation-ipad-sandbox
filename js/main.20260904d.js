...
(function (RT) {
  ...
  function bind() {
    ...
    function paperOffsetAtPoint(x, y) {
      var paper = $('#paper'); if (!paper) return null;
      var walker = document.createTreeWalker(paper, NodeFilter.SHOW_TEXT, null);
      var node, acc = 0, bestOff = null, bestDist = Infinity;
      while ((node = walker.nextNode())) {
        var len = node.nodeValue ? node.nodeValue.length : 0;
        if (node.parentElement && closestEl(node.parentElement, '#selLayer')) continue;
        for (var i = 0; i < len; i++) {
          var cr = document.createRange(); cr.setStart(node, i); cr.setEnd(node, i + 1);
          var rr = cr.getBoundingClientRect(); if (!rr.width && !rr.height) continue;
          var cx = rr.left + rr.width / 2, cy = rr.top + rr.height / 2;
          var dx = cx - x, dy = cy - y;
          var dist = dx * dx + dy * dy;
          if (dist < bestDist) {
            bestDist = dist;
            bestOff = acc + i + (x > rr.left + rr.width / 2 ? 1 : 0);
          }
        }
        acc += len;
      }
    ...
    function pointFromPaperOffset(off) {
      ...
      while ((node = walker.nextNode())) {
        if (node.parentElement && closestEl(node.parentElement, '#selLayer')) continue;
        var len = node.nodeValue ? node.nodeValue.length : 0;
        if (off <= acc + len) return { node: node, offset: Math.max(0, off - acc) };
        acc += len;
      }
      return null;
    }
    ...
    function paintHighlight(aNode, aOff, bNode, bOff) {
      ...
      startHandle.style.left = (firstRect.left - pr.left + paper.scrollLeft) + 'px';
      startHandle.style.top = (firstRect.top - pr.top + paper.scrollTop) + 'px';
      ...
      endHandle.style.left = (lastRect.right - pr.left + paper.scrollLeft) + 'px';
      endHandle.style.top = (lastRect.bottom - pr.top + paper.scrollTop) + 'px';
    ...
    document.addEventListener('touchstart', function (e) {
      ...
      if (tpHitToken) { clearTimeout(lpTimer); tpAnchor = null; return; }
      ...
      lpTimer = setTimeout(function () {
        ...
        if (!paintTokenAtPoint(tpAnchor)) RT.util.toast('请长按一段普通正文');
      }, 350);
    }, { passive: true });
    ...
    document.addEventListener('touchend', function (e) {
      ...
      clearTimeout(lpTimer);
      if (activeHandle) {
        activeHandle = null; tpAnchor = null; tpHitToken = null;
        suppressPaperClickUntil = Date.now() + 800;
        return;
      }
    ...
    document.addEventListener('touchcancel', function () {
      clearTimeout(lpTimer);
      tpAnchor = null; tpHitToken = null; tpDragging = false; tpLongPress = false; activeHandle = null;
      var p = $('#paper'); if (p) p.classList.remove('dragging');
      clearSelPop(); clearHighlight();
    }, { passive: true });
